...
const FAQItem = ({ item, open, onToggle }) => (
  <div className={`border border-fuchsia-500/15 rounded-xl overflow-hidden bg-gradient-to-br from-[#1d0b32]/85 to-[#2b0f4d]/80 backdrop-blur-xl shadow-lg shadow-black/40 transition ${open? 'accordion-open' : ''}`}>
    <button onClick={onToggle} className="w-full flex items-center justify-between px-5 py-4 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-fuchsia-500/60">
      ...
      <span className={`w-7 h-7 flex items-center justify-center rounded-full text-white text-xs font-semibold transform-gpu transition-transform duration-300 bg-gradient-to-br from-fuchsia-500 to-purple-600 shadow ${open ? 'rotate-45' : ''}`}>+</span>
    </button>
    <div className="accordion-content px-5 pb-5 pt-0 text-purple-200/80 text-sm leading-relaxed">{item.a}</div>
  </div>
);
...
const FAQSection = () => {
  ...
  <h2 className="animate-fade-slide-up text-3xl md:text-4xl font-bold text-center mb-5 bg-gradient-to-b from-white to-purple-200/70 bg-clip-text text-transparent">Answers</h2>
